...
<script type="text/javascript" src="easyui/1.3.4/locale/easyui-lang-zh_CN.js"></script>
	...
	<script type="text/javascript">
		...
		function openAdd(){
			$('#winadd').window('open');
			$('#winadd').window('refresh','topersonadd');
		}
		
		//打开编辑窗口
		function openEdit(id){
			$('#winedit').window('open');
			$('#winedit').window('refresh','topersonedit?id='+id);
		}
		 ...
		 */
		 ...
		 function showdata(){
			 $('#wu-datagrid-2').datagrid({
					url : 'selperson',
					loadFilter : pagerFilter,//分页选择器  不需要自己查分页的数据  查询所有数据就可以了
					rownumbers : true,
					singleSelect : false,
					pageSize : 10,
					pagination : true,
					...
					columns : [ [
							...
							},
							{
								...
								formatter : function(value, rows, index) {
									var str = '';
									str += '<a href="javascript:void(0)" class="user-easyui-linkbutton-edit" data-options="plain:true,iconCls:\'icon-edit\'" onclick="openEdit('+rows.id+');" >编辑</a>';
